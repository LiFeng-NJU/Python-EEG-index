# -*- coding: utf-8 -*-
"""
Created on Sun Dec 20 15:55:38 2020
...
def calculate():
    for channel_number in range(0,30,1):
        PESerial = []
        for TimeWindows in range(150,750,1):
            ts = result_1[TimeWindows:TimeWindows+50,channel_number]
            OrdinalPatternsSerial = ordinal_patterns(ts, 5, 1)
            #ShannonEntropy = s_entropy(OrdinalPatternsSerial)
            PE = p_entropy(OrdinalPatternsSerial)  
            PESerial.append(PE)
        plt.cla()
        plt.plot(PESerial)
        plt.savefig(str(channel_number)+'lie_25.png', dpi=200, bbox_inches='tight')
ERP_image()
#calculate()