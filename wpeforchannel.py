# -*- coding: utf-8 -*-
"""
Created on Mon Dec 21 10:32:22 2020
...
def calculate():
    for channel_number in range(0,30,1):
        ...
        for zeros in range (0,50,1):
            PESerial.append(0)
        '''
        for TimeWindows in range(0,500,100):
            ts = result_1[TimeWindows:TimeWindows+200,channel_number]
            OrdinalPatternsSerial = weighted_ordinal_patterns(ts, 8, 4)
        ...
        #plt.ylim(-0.2,1)
        plt.plot(PESerial)
        plt.savefig(str(channel_number)+'lie_WPE.png', dpi=200)

sum_1 = np.zeros(shape=(800,30))
for samples in range (10, 11, 1):
